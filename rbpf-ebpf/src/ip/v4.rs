...
pub fn handle_ingress_v4(ctx: &ContextWrapper) -> u32 {
    let ret = match ctx.to_parse_result(true, true) {
        Ok(ret) => ret,
        Err(proto) => {
            return {
                let proto = ipproto::as_u8(&proto);
                logs::send_err_unhandled_protocol("UNKNOWN IN v4", proto);
                xdp_action::XDP_DROP
            };
        }
    };
...
pub fn handle_egress_v4(ctx: &ContextWrapper) -> i32 {
    let ret = match ctx.to_parse_result(true, false) {
        Ok(ret) => ret,
        Err(proto) => {
            return {
                let proto = ipproto::as_u8(&proto);
                logs::send_err_unhandled_protocol("UNKNOWN OUT v4", proto);
                TC_ACT_SHOT
            };
        }
    };
