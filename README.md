...
`http_api` - Блок настроек HTTP REST API.

`on` - Включает / выключает HTTP сервер.

`addr` - Адрес на котором биндится HTTP сервер.

`port` - Порт на котором биндится HTTP сервер.
